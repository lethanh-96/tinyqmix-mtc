...
class Network:
    ...
    def __compute_collision(self, action):
        ...
        # iteratively detect collision
        for i in range(self.args.n_node):
            if self.nodes[i].is_transmitting:
                for j in range(i + 1, self.args.n_node):
                    if self.nodes[j].is_transmitting and selected_rbs[i] == selected_rbs[j]:
                        collision[i] = 1
                        collision[j] = 1
        self.action = action
        return collision
    ...
    def set_rbs(self, rbs):
        for i, rb in enumerate(rbs):
            self.nodes[i].set_rb(rb)
    # =====================================================================================
